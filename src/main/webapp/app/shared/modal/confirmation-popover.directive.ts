...
import {ConfirmationPopoverOptions, ConfirmationPopoverWindowOptions} from './confirmation-popover-options.provider';
...
export interface OnPopoverEvent {
 ...
 */
@Directive({
    selector: '[cjtjConfirmationPopover]'
})
export class ConfirmationPopoverDirective implements OnDestroy, OnChanges, OnInit {
    ...
    @Output() onPopover: EventEmitter<OnPopoverEvent> = new EventEmitter();
    ...
    private showPopover(): void {
        if (this.onBeforePopover != null && !this.onBeforePopover()) {
            return;
        }
        if (!this.popover && !this.isDisabled) {
            ...
            const componentFactory: ComponentFactory<ConfirmationPopoverWindowComponent> = this.cfr.resolveComponentFactory(ConfirmationPopoverWindowComponent);
