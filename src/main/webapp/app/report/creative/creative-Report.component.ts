...
@Component({
    selector: 'cjti-creativeReport',
    templateUrl: './creative-Report.component.html',

})
export class CreativeReportComponent implements OnInit {
    account: any; // 当前账户
    ...
    loadCreativeByGroupId(campaignId:number,groupId:number, startTime: string, endTime: string,syn:boolean ) {
        ...
        this.creativeService.getCreativesReport(campaignId,groupId,startTime,endTime,syn).subscribe(
            response=>{
                this.creatives= response.results;
                this.allCreativeReport=response.allCreativeReport;
                this.query();
            }
        )
     ...
     */
    query() {
        this.changeCreativesChecked(this.creatives, false);

        const filteredCreatives =
            this.creatives.filter((creative: any) =>
                this.queryParams.creativeName === '' || creative.info.creativeName.includes(this.queryParams.creativeName));
    ...
    batchDeleteCreatives() {
        ...
        this.creativeService.delCreative(creativeModels).subscribe(
            (response) => {
                if (response.success) {
                    this.loadCreativeByGroupId(1,1,null,null,true);
                }else {
